...
class Hotel:
    ...
    def make_reservation(self, guest_name: str, description: str,
        ...
        reserva = Reservation(guest_name, description, check_in, check_out)
        self.rooms[room_number].book(reserva.id, check_in, check_out)
        self.reservations[reserva.id] = reserva
        return reserva.id

    def add_guest(self, reservation_id: str, name: str, email: str, type_: str):
        reserva = self.reservations.get(reservation_id)
        if not reserva:
            reservation_not_found_error()
        else:
    ...
    def find_available_rooms(self, check_in: date, check_out: date) -> list[int]:
        ...
        for num, room in self.rooms.items():
            ...
            while curr < check_out:
                if room.availability.get(curr) is not None:
                    libre = False
                    break
    ...
    def add_service_to_reservation(self, reservation_id: str, service_name: str, price: float):
        reserva = self.reservations.get(reservation_id)
        if not reserva:
            reservation_not_found_error()
        else:
            servicio = HotelService(service_name, price)
            reserva.add_service(servicio)
